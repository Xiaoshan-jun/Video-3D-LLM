...
def accuracy_reward(completions, solution, **kwargs):
    def extract_answer(text):
        ...
        if match:
            return match.group(
                1).strip()  # remove leading and trailing whitespace, so we get the first clean context inside the pattern
        return ""
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                if ref_words[i - 1] == hyp_words[j - 1]:
                    d[i][j] = d[i - 1][
                        j - 1]  # if the head is the same, the distance is lev distance of the following distance
                else:
                    d[i][j] = 1 + min(d[i - 1][j], d[i][j - 1], d[i - 1][
                        j - 1])  # if the head is different, distance is 1 plus min of (tail(a), b), (a, tail(b), and tail(a), tail(b))
    ...
    def compute_rouge_score(reference, hypothesis, use_stemmer=True):
        ...
        scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'],
                                          use_stemmer=use_stemmer)  # use rouge score calculator
            ...
            if question_type == "multiple choice":
                ...
                if gt_has_decimal != out_has_decimal:
                    ...
                    if gt_number is None or out_number is None:
                        ...
                        reward = 1.0 if round(gt_number, 2) == round(out_number,
                                                                     2) else 0.0  # reward 1 only if the number is same as out number , rou
                ...
                score = compute_rouge_score(gt_ans,
                                            output_ans)  # automatic metric to evaluate overlap between free-form answer and reference
