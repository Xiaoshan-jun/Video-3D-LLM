...
#Union is a type support, so a variable can be different type
...
class Qwen2VLGRPOVLLMTrainerModified(Trainer):
    def __init__(
        ...
        script_args = None,
        # Optional means a value can be None or Type
        ...
        ] = None, #List of tokenizers or processors used to prepare inputs for the reward funcs
        callbacks: Optional[list[TrainerCallback]] = None,
        optimizers: tuple[
            Optional[torch.optim.Optimizer], Optional[torch.optim.lr_scheduler.LambdaLR]
        ] = (None, None),
        peft_config: Optional["PeftConfig"] = None, #Parameter-Efficient Fine-Tuning
        ...
        #each reward functions have corresponding preprocess function
    ...
    def compute_loss(
        ...
        prompts_text = [
            maybe_apply_chat_template(example, self.processing_class)["prompt"] #utility function transfer prompt to string
            for example in inputs
        ]

        #-------------------------------use first example to prepare the prompt structure________________________
        ...
        mm_data = [[data_type, image_inputs if image_inputs else video_inputs]] # image/video data
        prompt_inputs = super()._prepare_inputs(prompt_inputs) #move it to device
        prompt_ids, prompt_mask = prompt_inputs["input_ids"], prompt_inputs["attention_mask"] #get the token ids and mask
        ...
        #if temporal, shuffle the video frame
        if self.temporal:
            if video_inputs: #only apply for video
                indices = torch.randperm(video_inputs[0].size(0)) #generate random permutation of frame indices
                shuffled_video_inputs = [video_inputs[0][indices]] #shuffle frame order
        ...
        #vitrualized LLM inference engine(fast and memory-efficient generation of text from large language models), it generate the text faster than the original model
        #during training, we need to make sure the vllm is synced with the current model.
        if self.args.use_vllm:
            #
            # First, have main process load weights if needed
            if self.state.global_step != self._last_loaded_step: #check if weights need to be synced
            ...
            # Create sampling params with num_generations, generate multiple completions with vllm
            if self.accelerator.is_main_process:
